...
void TMSelectServerScene::InitializeUI()
{
	...
	if (m_pNServerList)
	{
		...
		// Dead Chicken: the channel list control gets width 0 when its texture
		// set fails to load (custom UI entries absent in our package) - force
		// sane dimensions so the populated list is visible and clickable.
		if (m_pNServerList->m_nWidth <= 0 && m_pNServerGroupList)
		{
			m_pNServerList->m_nWidth = m_pNServerGroupList->m_nWidth;
			m_pNServerList->m_nHeight = m_pNServerGroupList->m_nHeight;
		}
	}
